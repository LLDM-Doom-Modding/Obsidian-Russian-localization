...
#if DEBUG_BLOCKMAP
    DebugPrint("BlockAddLine: %d (%d,%d) -> (%d,%d)\n", line_index, x1, y1, x2, y2);
#endif
...
#if DEBUG_BLOCKMAP
    DebugPrint("Blockmap: Last ptr = %d  duplicates = %d\n", cur_offset, dup_count);
#endif
...
#if DEBUG_BLOCKMAP
    DebugPrint("Blockmap lines centered at (%d,%d)\n", block_mid_x, block_mid_y);
#endif
}
...
void PutBlockmap()
{
    ...
    else
    {
        ...
        LogPrint("    Blockmap size: %dx%d (compression: %d%%)\n", block_w, block_h, block_compression);
    }
...
#if DEBUG_LOAD
    DebugPrint("GetThingsHexen: num = %d\n", count);
#endif
...
#if DEBUG_LOAD
    DebugPrint("GetLinedefsHexen: num = %d\n", count);
#endif
...
void ParseVertexField(vertex_t *vertex, const std::string &key, ajparse::token_kind_e kind, const std::string &value)
{
    if (key == "x")
...
void ParseSectorField(sector_t *sector, const std::string &key, ajparse::token_kind_e kind, const std::string &value)
{
    // nothing actually needed
}

void ParseSidedefField(sidedef_t *side, const std::string &key, ajparse::token_kind_e kind, const std::string &value)
{
    if (key == "sector")
    {
...
void ParseLinedefField(linedef_t *line, const std::string &key, ajparse::token_kind_e kind, const std::string &value)
{
    if (key == "v1")
        line->start = SafeLookupVertex(ajparse::LEX_Int(value));

    if (key == "v2")
        line->end = SafeLookupVertex(ajparse::LEX_Int(value));

    if (key == "special")
        line->type = ajparse::LEX_Int(value);

    if (key == "twosided")
        line->two_sided = ajparse::LEX_Boolean(value);
...
void PutGLVertices(int do_v5)
{
    ...
    if (count != num_new_vert)
        FatalError("PutGLVertices miscounted (%d != %d)\n", count, num_new_vert);
}
...
#if DEBUG_BSP
        DebugPrint("PUT SUBSEC %04X  First %04X  Num %04X\n", sub->index, LE_U16(raw.first), LE_U16(raw.num));
#endif
    }
...
#if DEBUG_BSP
        DebugPrint("PUT V3 SUBSEC %06X  First %06X  Num %06X\n", sub->index, LE_U32(raw.first), LE_U32(raw.num));
#endif
    }
...
static void PutOneNode_V5(node_t *node, Lump_c *lump)
{
    ...
    if (node->r.node)
        ...
        FatalError("Bad right child in V5 node %d\n", node->index);
...
static void AddMissingLump(const char *name, const char *after)
{
    ...
    if (exist < 0)
    {
        LogPrint("Missing %s lump -- level structure is broken\n", after);
